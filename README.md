...
A cozy portrait mobile game where fairytale customers visit your magical bubble shop with a wish — mix bubbles, charms, and ingredients to make each wish come true before their patience pops!

## ▶️ Play the game

**https://NORS3AI.github.io/Wish-Pop-Bubble-Shop/**

> Note: this link is case sensitive. It must match the repository name exactly,
> capitalization included.

## How to play

- A fairytale customer arrives with a wish shown as a little recipe of colored bubbles and charms.
- Tap the ingredients at the bottom to add them to your cauldron (tap a bubble in the cauldron to remove it).
- Hit **✨ Serve** when your mix matches the wish. Match it before their patience bar runs out!
- Chain correct wishes for **combo** bonuses, earn coins, and reach new days as the recipes get trickier.
- You have 3 hearts — lose one each time a customer leaves unhappy.
...
- 🔎 **Catch Lady Gothel in the act.** She's a menace — and whenever she stops by your shop she
  ...
  her results — so you never know when to expect it. You've got five seconds to catch it before it
  poofs away (it'll turn up again next time), and everything else pauses while you chase it, so you
  won't fumble an ingredient by accident.
...
- 🃏 **The Copycat (early test).** A mischievous new mechanic in the works: every ingredient you
  ...
  something: a random magic that either **adds to** its bar (+) or **drains** it (−), so the mirror
  is a moving target you read and time. You can try a practice round from the testing menu
  (Admin → *Copycat Round*); it loops so you can experiment.
...
- 🪵 **A hidden stash in the wall.** The popping phase now plays against a cozy wooden wall.
  ...
  it: a bit of gold, Stardust, or — if you're lucky — a rare Pearl.
...
- 🐷🐷 **Meet Thatch & Woody.** The straw-house worrier (**Thatch**) and the stick-house denier
  (**Woody**) now have real hand-drawn art with a full range of expressions — hopeful, thrilled,
  grumpy-and-covered-in-straw, even sneezy — so you see exactly how each wish landed. Their
  **Moving Day** scene now plays out fully: the pair arrive in tears, shrug it off, and — in a framed
  storybook portrait — reveal their fancy brick-building brother **Mason Piggleby** (top hat, monocle,
  the works) before packing a whole house into one small suitcase.
...
- 🐭 **New Courtyard game — Mouse Boutique!** A cozy "keep the shop running" game, now set in a
  ...
  dress** and it scurries to its next table, and you watch it **take shape stage by stage** (a bolt of
  fabric → cut pieces → a sewn gown → beaded → all trimmed and finished!). Each dress shows the tables
  it still needs right above it. Every table holds **one dress at a time**, so keep the line moving —
  and if a table's busy, tapping a dress hops it back to the shelf so you can unclog. Deliver enough
  gowns before too many customers storm off. Easy / Medium / Hard. Play it as a King's Courtyard
  event or from the gear menu.
...
- 🎂 **Mirrored patterns + right-sized treats.** Each tier's treats are now a **symmetrical
  ...
  circle** while shrinking on the busier tiers so they no longer overlap each other.
...
- 🖼️ **The customer screen is dialed in!** Characters now sit comfortably in the wide arch (no more
  ...
  showing just **Payment · Target · Allergy** — the allergy is a quick mark (✕ none, ✔ one, ✔✔ two).
  All over a cozy village backdrop with a grand green **Start Scoop** button.
...
- 🎀 **The home screen got a full makeover!** New top bar with your buddy in the corner and your
  ...
  that opens your daily quests (with a badge when rewards are ready). Admin & testing now tucks
  behind the ⚙️ gear in the corner.
...
- 📋 **Round Recap + fixes.** The result screen now shows a **recap** of your round:
  bubbles scooped, bubbles popped (including bonus), ingredients found, charms found,
  gold/treats gained, and triples made. Also: an allergic reaction now correctly costs
  you pay (**no tips on a reaction**), and **Pop‑them‑all** cleanly sweeps every
  ingredient and charm into your bag/tray.
