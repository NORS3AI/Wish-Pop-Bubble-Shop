...
const D = DATA;
const BUILD = "v595"; // bump on each deploy; shown on the start screen to verify the live version
...
// The Stepsisters — the Courtyard's version of Bo Peep. They bicker their way through
// asking you to find their lost beads; meeting them turns ON the bead hunt.
function playStepsisters() {
  SFX.unlock();
...
function maybeStepsisters() {
  if (GAME.realm !== "courtyard" || GAME.stepsistersMet) return false;
  if (servedTotal < pacing("stepsistersAfter", 2)) return false;   // let the player settle into the realm first
  playStepsisters(); return true;
}
// If you didn't win the Beaded Cauldron, the Stepsisters keep squabbling their way back — the
// re-string minigame pops up again at random in the Courtyard (the fight-over-the-necklace
// cutscene straight into the game, NOT the whole quest). Once you win the pot, it never returns.
function maybeBeadRematch() {
  if (GAME.realm !== "courtyard") return false;
  if (GAME.huntCelebrate === "courtyard") return false;        // the first invitation hasn't played yet
  const st = GAME.hunts && GAME.hunts.courtyard;
  if (!st || !st.done) return false;                            // must have finished the bead hunt first
  if (GAME.owned && GAME.owned.cauldron_pearl) return false;    // already won the pot → never again
  if (GAME.beadRematchAt == null) { GAME.beadRematchAt = servedTotal + R.int(3, 5); save(); return false; }  // cool-off after the last try
  if (servedTotal < GAME.beadRematchAt) return false;
  if (!R.chance(0.5)) return false;                             // once due, a coin-flip each round so it feels random
  GAME.beadRematchAt = servedTotal + R.int(3, 5); save();       // schedule the next possible try
  playBeadCutscene(renderStart);
  return true;
}
...
const HUNTS = {
  ...
  courtyard: { realm: "courtyard", char: "the Stepsister", charEmoji: "💃", item: "bead", itemEmoji: "📿",
    need: 8, chance: 0.25, skin: "cauldron_pearl", deferSkin: true,   // the pot is the minigame WIN prize, not an auto-gift for finding the beads
...
function huntUnlocked(realm) { return realm === "willow" ? !!GAME.bopeepMet : realm === "courtyard" ? !!GAME.stepsistersMet : true; }
...
function huntComplete(h) {
  ...
  GAME.owned = GAME.owned || {};
  // Most hunts hand over their skin the moment the set is complete. The Courtyard defers it
  // (deferSkin) — you only earn the Beaded Cauldron by WINNING the re-string minigame.
  if (!h.deferSkin) {
    GAME.owned[h.skin] = true;
...
function maybeShowHuntCelebrate() {
  ...
  if (realm === "courtyard") {
    ...
    // Cutscene → minigame. Win/lose reaction + the Beaded Cauldron prize are handled by
    // beadsOutro() when the player finishes the minigame.
    playBeadCutscene(renderStart);
    return;
  }
...
function renderAdmin() {
  ...
  on("#ad-wine", "click", playWineStory);
  ...
  on("#ad-courtyard-reset", "click", () => {
    ...
    GAME.stepsistersMet = false;   // the Stepsisters ask again (re-locks + replays the lost-bead intro)
    GAME.beadRematchAt = null;     // clear the rematch schedule
    GAME.nextEventAt = -1;
    save();
...
function resolveEventToken(token, plan, idx) {
  ...
  if (token === "wine") return playWineStory;
...
const GOB_HINT = false;      // outline the matching bin tile? off for now (may switch back on later)
...
// A little story-mode arrival — Gribble sizes you up, whines about his empty
// belly, then strikes the feed-me-junk deal. Poses cycle through his talkers.
const GOB_STORY = [
  { fig: "gribble_hold", text: "“Oi! Down 'ere, in the muck! Name's <b>Gribble</b> — royal rubbish-gobbler, finest in the kingdom. Ooh… is that a <b>whole bin</b> of lovely junk yer haulin'?”" },
  { fig: "gribble_think", text: "“I ain't ate in <i>days</i>. Been dreamin' o' banana peels an' soggy boots an' crunchy fish bones… me belly's growlin' louder'n a troll with a toothache.”" },
  { fig: "gribble_point", cta: "😋 Feed him!", text: "“'Ere's the deal: I <b>call out</b> the junk I fancy, you <b>toss the match in me gob</b> — quick now! Stuff me full an' I'll pay ya in shinies. Miss too many an' I'm off. Deal?”" },
];
let GOB_STORY_I = 0;
function goblinStoryBeat() {
  const b = GOB_STORY[GOB_STORY_I];
  html("event", `
    ${hud("A Hungry Goblin!")}
    <div class="gob-intro">
      <div class="gob-intro-bg" style="background-image:url('art/gribble_bg.webp?v=${BUILD}')"></div>
      <div class="gob-intro-fig">${ART.tag(b.fig, "🧌", "gob-intro-img")}</div>
      <div class="gob-intro-card">
        <div style="font-weight:800;font-size:20px">Gribble the Goblin</div>
        <div class="speech">${b.text}</div>
        <button class="btn good" id="gob-next">${b.cta || "Continue  ▸"}</button>
        ${GOB_STORY_I === 0 ? `<button class="btn secondary" id="gob-skip" style="margin-top:8px">Not now</button>` : ""}
      </div>
    </div>
  `);
  on("#gob-next", "click", () => { GOB_STORY_I++; if (GOB_STORY_I < GOB_STORY.length) goblinStoryBeat(); else goblinBegin(); });
  on("#gob-skip", "click", startRound);
  show("event");
}
function goblinBegin() {
  GOBLIN = { bin: gobMakeBin(), want: null, fake: false, fed: 0, miss: 0, answered: true, timer: null, cdTimer: null, feedback: null, countdown: null };
  goblinCountdown();
}
// 3-2-1 drawn right over the live game UI so you see the board before it starts.
function goblinCountdown() {
  ...
  const tick = () => {
    ...
    else { GOBLIN.cdTimer = setTimeout(() => { if (GOBLIN) { GOBLIN.countdown = null; goblinRequest(); } }, 420); }
  };
  tick();
}
...
function gobFlyToMouth(id, tileEl) {
  ...
  // 1) pop up in place, then 2) arc up into his gob, shrinking away to nothing
  requestAnimationFrame(() => {
    fly.style.transition = "transform .15s cubic-bezier(.3,1.6,.5,1)";
    fly.style.transform = "translate(-50%,-50%) scale(1.55)";
  });
  setTimeout(() => {
    fly.style.transition = "left .42s cubic-bezier(.4,0,.7,1), top .42s cubic-bezier(.25,-.5,.7,1), transform .42s ease-in, opacity .42s ease-in .12s";
    fly.style.left = dx + "px"; fly.style.top = dy + "px";
    fly.style.transform = "translate(-50%,-50%) rotate(165deg) scale(.08)";
    fly.style.opacity = "0";
  }, 165);
  setTimeout(() => fly.remove(), 640);
}
function renderGoblin() {
  const counting = GOBLIN.countdown != null;
  const want = GOBLIN.want, wantIt = want ? GOB_BY_ID[want] : null;
  // GOB_HINT off: the tiles are NOT outlined, so you have to spot the match yourself.
  const tiles = GOB_ITEMS.filter(it => GOBLIN.bin[it.id] > 0).map(it =>
    `<button class="gob-tile${(GOB_HINT && !counting && it.id === want) ? " wanted" : ""}" data-id="${it.id}">
       <span class="gob-tile-img">${gobArt(it.id, "")}</span><span class="gob-count">×${GOBLIN.bin[it.id]}</span>
     </button>`).join("");
  const fb = GOBLIN.feedback;
  const fbTxt = counting ? "Get ready…"
    : !fb ? `He wants: <b>${wantIt.name}</b>!`
  ...
  const wantInner = counting || !want ? `<span class="gob-want-q">?</span>` : gobArt(want, "gob-want-img");
  const overlay = counting ? `<div class="gob-count-big ${GOBLIN.countdown === 0 ? "go" : ""}">${GOBLIN.countdown > 0 ? GOBLIN.countdown : "Feed!"}</div>` : "";
  html("event", `
    ${hud("Feed Gribble!")}
    <div class="gob-stage mg-fullbleed" id="gob-stage">
      ...
      <div class="gob-want-box"><div class="gob-want-frame">${wantInner}</div></div>
      <div class="gob-timer"><i id="gob-timerbar"></i></div>
      <div class="gob-fb" id="gob-fb">${fbTxt}</div>
      <div class="gob-grid" id="gob-grid">${tiles}</div>
      <button class="btn secondary gob-nope" id="gob-nope"${counting ? " disabled" : ""}>🚫 Don't have it!</button>
      ${overlay}
    </div>
  `);
  if (!counting) {
  ...
  if (tb && !counting) { const ms = gobMs(); tb.style.transition = "none"; tb.style.width = "100%";
    requestAnimationFrame(() => { if (tb.isConnected) { tb.style.transition = "width " + ms + "ms linear"; tb.style.width = "0%"; } }); }
  else if (tb) { tb.style.transition = "none"; tb.style.width = "100%"; }
}
...
function feastSpawnFromWheel() {
  ...
  // fall down a central band (evenly spread, pulled well in from the screen edges — no lanes)
  feastAddFaller(pick.kind, 24 + Math.random() * 52, pick.y);
}
...
function stackPlay() {
  ...
  // Handle both touch and pointer input; preventDefault on touchstart/move keeps iOS
  // Safari from hijacking the swipe as a scroll/selection gesture.
  const fromTouch = e => { const t = (e.touches && e.touches[0]) || (e.changedTouches && e.changedTouches[0]); if (t) move(t.clientX); };
...
// Two story beats to set the scene (Jasper fretting over the toast), then the how-to + difficulty.
function playWineStory() {
  SFX.unlock();
  ["jester_think", "jester_worried", "jester_excited", "jester_serve", "jester_fumble", "jester_spill"].forEach(k => ART.ensure(k, () => {}));
  renderStoryBeats([
    { name: "Jasper the Jester", fig: "jester_think", bg: "courtyard_mid", text: "The King demands a grand toast — so I'm to pour the enchanted <b>Wish-Wine</b> <i>and</i> juggle for the whole court. At the very same time. Hmm… how hard could it possibly be?" },
    { name: "Jasper the Jester", fig: "jester_worried", bg: "courtyard_mid", cta: "Help him!  ▸", text: "Oh <b>dear</b>. It's already dribbling down the King's cloak — and my juggling balls won't stay up! Every ball I drop knocks over <i>another</i> goblet! Quick, help me — dab the drops <b>and</b> keep my balls flying!" },
  ], renderWineIntro);
}
...
function wineFinish(win, why) {
  ...
  if (win) {
    ...
    grantReward({ gold, stardust: dust }); save();
    SFX.perfect(); SFX.bigCoin(); confettiOver($("#app"));
    // win image = whichever half you did BETTER at: fewer dropped balls → juggling; else the wine
    outcome = { emoji: "🍷", jester: dropped < stains ? "jester_excited" : "jester_serve", title: "Cloak saved!", cls: "win",
    ...
    // lose image = whatever undid you: all balls down → dropped-balls Jester; else the spilled-wine Jester
    outcome = { emoji: ballsOut ? "🤹" : "🟣", jester: ballsOut ? "jester_fumble" : "jester_spill", title: ballsOut ? "The juggling collapsed!" : "The cloak's ruined!", cls: "lose",
  ...
  `);
  on("#wine-next", "click", () => wineOutro(outcome.jester, win));
  show("event");
}
// One story-mode slide after the toast: Jasper reacts with the pose that fits how it went —
// won juggling / won serving, or dropped balls / spilled wine.
function wineOutro(jesterKey, win) {
  ART.ensure(jesterKey, () => {});
  const lines = {
    jester_excited: "Ha-<b>HA</b>! Not a drop spilled and every ball still aloft — the whole court is on its feet! You magnificent creature, you.",
    jester_serve:   "The King's cloak — <b>spotless</b>! And every goblet poured without a splash. Take a bow with me, my friend!",
    jester_fumble:  "My balls! Every last one — on the <i>floor</i>! The act came crashing down… let's try again before the King stops laughing <i>at</i> me.",
    jester_spill:   "Catastrophe! Wine <b>everywhere</b> — the King's cloak is ruined and his face has gone purpler than my hat. Quick — another go!",
  };
  renderStoryBeats([
    { name: "Jasper the Jester", fig: jesterKey, bg: "courtyard_mid", cta: win ? "Huzzah!  ▸" : "Try again  ▸", text: lines[jesterKey] || "" },
  ], startRound);
}
...
let beadsWonThisVisit = false;   // won a full set at least once this visit → the sisters are pleased + you claim the pot
const BEAD_TICK = 40;
...
function renderBeadsIntro(done) {
  ...
  BEADS = null; beadsWonThisVisit = false;   // fresh visit — earn the pleased reaction (and the pot) anew
...
function beadsFinish(won) {
  ...
  if (won) beadsWonThisVisit = true;   // full set → the sisters are pleased and you've earned the pot
  ...
  on("#beads-done", "click", () => beadsOutro(beadsWonThisVisit));
  show("event");
  beadsPhotoLayout();
}
...
// After the minigame: the sisters react (pleased if you finished a full set, sour if not), then —
// on a win — hand over the Beaded Cauldron. `won` = completed at least one full set this visit.
function beadsOutro(won) {
  const d = beadsDone; beadsDone = null; BEADS = null;
  const after = d || renderStart;
  const fig = won ? "stepsisters_greet" : "stepsisters_sulk";
  ART.ensure(fig, () => {});
  renderStoryBeats([
    won
      ? { name: "The Stepsisters", fig: "stepsisters_greet", figClass: "ss-duo-big", bg: "courtyard_mid", cta: "Take your prize  ▸",
          text: "<span class='speak-p'>You strung every last one — perfectly!</span><br><span class='speak-g'>Hmph. Better than <i>we</i> could’ve. Oh, don’t look so smug… here, you’ve earned this.</span>" }
      : { name: "The Stepsisters", fig: "stepsisters_sulk", figClass: "ss-duo-big", bg: "courtyard_mid", cta: "Back to the shop  ▸",
          text: "<span class='speak-p'>Ugh — they’re all in the <i>wrong</i> order!</span><br><span class='speak-g'>Hopeless. Come back when you can string a <b>proper</b> necklace.</span>" },
  ], () => {
    if (won) grantSkin("cauldron_pearl", { kicker: "The Stepsisters’ prize!", onDone: after });
    else after();
  });
}
...
function danceWarmup() {
  ...
  // Same skeleton as the live dance (grace bar + cue + beat-track kept as hidden
  // spacers) so the dancer sits at the exact same size and height — no "jump up"
  // when Start is pressed.
  html("event", `
    ${hud("Warm Up 💃")}
    <div class="dance-stage mg-fullbleed">
      <button class="btn good dance-start" id="dance-start">▶ Start the lesson!</button>
      <div class="dance-grace top" style="visibility:hidden"><i style="width:0%"></i></div>
      ...
      <div class="dance-announce">
        <div class="dance-cue">&nbsp;</div>
        <div class="dance-sub">Tap the four moves to try them. Press <b>Start</b> above when you're ready!</div>
      </div>
      <div class="beat-track" style="visibility:hidden"></div>
...
function renderDanceIntro(partnerId) {
  ...
  on("#dance-play", "click", () => {
    DANCE = { p, phase: 1, routine: danceRoutine(p.steps), idx: 0, score: 0, nailed: 0,
      total: p.steps, answered: true, feedback: null, armTime: 0, beatTime: 0,
...
function danceStep() {
  ...
  if (DANCE.idx >= DANCE.routine.length) {
    if (DANCE.phase === 1) { danceRehearsalDone(); return; }
    danceFinish(); return;
  }
...
// practice it again as many times as they like, or head to the ball for the
// real, graded dance-from-memory. (Only the ball counts toward the score, so a
// shaky rehearsal never hurts — and practice runs can be repeated freely.)
function danceRehearsalDone() {
  if (!DANCE) return;
  ...
  html("event", `
    ${hud("Rehearsal done!")}
    <div class="grow center" style="gap:16px">
      <div class="dance-face" style="font-size:76px">🎼</div>
      <div class="result-title win">You know the steps!</div>
      <p class="muted" style="max-width:300px">You've been through the whole routine. Rehearse it again to lock it in, or take ${p.name} to the ball and lead it <b>from memory</b>.</p>
      <button class="btn good" id="dance-final">🎼 Dance the ball!</button>
      <div style="height:8px"></div>
      <button class="btn secondary" id="dance-again">↻ Practice again</button>
    </div>
  `);
  on("#dance-final", "click", () => {
    if (!DANCE) return;
    DANCE.phase = 2; DANCE.idx = 0; DANCE.score = 0; DANCE.nailed = 0;
    DANCE.feedback = null; DANCE.pose = 1; DANCE.poseWorried = false;
    SFX.fanfare && SFX.fanfare();
    danceCountdown("Dance!", danceStep);
  });
  on("#dance-again", "click", () => {
    if (!DANCE) return;
    DANCE.idx = 0; DANCE.score = 0; DANCE.nailed = 0;
    DANCE.feedback = null; DANCE.pose = 1; DANCE.poseWorried = false;
    danceCountdown("memorize!", danceStep);   // same routine, rehearse it once more
  });
  show("event");
}
