...
const BALANCE = {
  ...
  INFUSED_NEED_BIAS: 0.65,       // chance a flex-infused ingredient's magic lands on a need (else off-target — a gamble, and no free peek at hidden needs)
  ...
  VIP_ENABLED: false,          // master switch for the ⭐ VIP key-wager feature (off for now)
...
function pointsForNeed(slots, type) {
  ...
  return Math.max(0, points);   // a bar can't drop below empty (minus reflections clamp here)
}
